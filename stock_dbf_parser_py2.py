...
# 股票实时行情 Level01 DBF文件解析工具(Python2.+)
...
def get_stock_sz_kline_data():
    ...
    while True:
        ...
        for record in db:
            ...
            # 解析头信息
            if temp == '000000':
                tr_date = str(record[1]).decode('GBK').strip()  # 日期
                cur = str(record[5]).decode('GBK').strip()  # 是否开市
                times = str(record[7]).decode('GBK').strip()  # 时间
                ex_zs = str(record[2]).decode('GBK').strip().split('\x00')[0]
                ex = float(ex_zs)  # 指数系数
            # cur:0 开市， cur:1 闭市
            elif cur == '0' and times != NOW_TIME and tr_date == TODAY and times:
                if temp.startswith('000') or temp.startswith('002') or temp.startswith('300') or temp.startswith('399'):
                    
                    data = {}
                    
                    if temp.startswith('399'):
                        # 标记为 深圳 指数
                        code = temp.decode('GBK').strip() + '.SZSH.ZS'
                        yclose = float(str(record[2]).decode('GBK').strip().replace(" ", '')) * ex  # 股票昨日收盘价
                        topen = float(str(record[3]).decode('GBK').strip().replace(" ", '')) * ex  # 股票开盘价
                        close = float(str(record[4]).decode('GBK').strip().replace(" ", '')) * ex  # 股票收盘价
                        high = float(str(record[8]).decode('GBK').strip().replace(" ", '')) * ex  # 股票最高价
                        low = float(str(record[9]).decode('GBK').strip().replace(" ", '')) * ex  # 股票最低价
                    else:
                        # 个股
                        code = temp.decode('GBK').strip() + '.SZ.GG'
                        yclose = float(str(record[2]).decode('GBK').strip().replace(" ", ''))  # 股票昨日收盘价
                        topen = float(str(record[3]).decode('GBK').strip().replace(" ", ''))  # 股票开盘价
                        close = float(str(record[4]).decode('GBK').strip().replace(" ", ''))  # 股票收盘价
                        high = float(str(record[8]).decode('GBK').strip().replace(" ", ''))  # 股票最高价
                        low = float(str(record[9]).decode('GBK').strip().replace(" ", ''))  # 股票最低价
                    
                    name = record[1].decode('GBK').strip()  # 股票简称
                    volume = float(str(record[5]).decode('GBK').strip())  # 股票成交量
                    amount = float(str(record[6]).decode('GBK').strip())  # 股票成交金额
...
def check_sh_data(data):
    ...
    try:
        ...
        print "[StockDBFUtil] check_sh_data Error:", e
    finally:
        client.close()
